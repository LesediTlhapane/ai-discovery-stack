...
function TrendsPage() {
  ...
  const getFallbackPosts = useCallback((): FeedPost[] => {
    ...
    return fallbackItems.slice(0, 15).map((item, index) => ({
      id: index + 5000,
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="relative group">
            ...
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 flex gap-0.5">
              <span className={`h-1 w-1 rounded-full transition-all ${theme === 'dark' ? 'bg-accent' : 'bg-white/30'}`} />
              <span className={`h-1 w-1 rounded-full transition-all ${theme === 'light' ? 'bg-accent' : 'bg-white/30'}`} />
              <span className={`h-1 w-1 rounded-full transition-all ${theme === 'image' ? 'bg-accent' : 'bg-white/30'}`} />
            </div>
            
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 text-[10px] bg-black/80 text-white px-2 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
              {getThemeLabel()}
            </div>
          </div>
...
export function AIDiscoveryApp() {
  ...
  // Apply theme
  useEffect(() => {
    document.documentElement.classList.remove("dark", "light", "image-bg");
    document.documentElement.classList.add(theme);
    localStorage.setItem("aidc-theme", theme);
  }, [theme]);
  
  // Toggle cycles: image → dark → light → image
  const toggleTheme = () => {
    setTheme(prev => {
      if (prev === "image") return "dark";
  ...
  // Ref for scrolling to first tool on search
  const firstToolRef = useRef<HTMLDivElement | null>(null);

  // Auto-scroll to first matching tool when search is performed
  useEffect(() => {
    if (query.trim().length > 0 && filtered.length > 0 && firstToolRef.current) {
      setTimeout(() => {
        firstToolRef.current?.scrollIntoView({
          behavior: 'smooth',
          block: 'start',
        });
      }, 100);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      {/* ===== BACKGROUND IMAGE - DIRECT IN JSX ===== */}
      {theme === "image" && (
          ...
          <>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-6">
              <div className="glass-strong rounded-2xl p-4">
                ...
                {activeDept && (
                  <div className="mt-3 text-xs text-fg-mute">
                    Showing tools for: <span className="text-accent font-semibold">{activeDept}</span>
                  </div>
                )}
              </div>
            </section>
              ...
              <div className="grid lg:grid-cols-[300px_minmax(0,1fr)] gap-6">
                ...
                <aside className="hidden lg:block lg:sticky lg:top-[88px] lg:self-start">
                  <div className="glass-strong rounded-2xl p-4 space-y-5">
                    {/* Categories */}
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-wider text-fg-strong flex items-center gap-2 mb-3">
                    ...
                    <div className="border-t border-[var(--surface-border)] pt-4">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-fg-strong flex items-center gap-2 mb-3">
                        <TrendingUp className="h-3.5 w-3.5 text-accent" /> Popular Tools
                      </h3>
                      <div className="relative">
                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-accent/30">
                          {trending.map((t) => (
                            <button
                              key={t.id}
                              onClick={() => setOpenTool(t)}
                              className="shrink-0 w-44 glass rounded-xl p-3 text-left hover:border-accent/40 hover:-translate-y-0.5 transition-all"
                            >
                              <div className="flex items-center gap-2">
                                <BrandLogo tool={t} size={32} />
                                <div className="min-w-0 flex-1">
                                  <p className="text-xs font-semibold text-fg-strong truncate">{t.name}</p>
                                  <p className="text-[10px] text-accent truncate">{t.categories[0]}</p>
                                </div>
                                <Star className="h-3 w-3 text-accent fill-accent shrink-0" />
                              </div>
                              <p className="mt-1 text-[10px] text-fg-soft line-clamp-2">{t.summary}</p>
                            </button>
                          ))}
                        </div>
                        <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-[var(--surface-strong-1)] to-transparent pointer-events-none" />
                      </div>
                    </div>

                    {/* Tags */}
                    <div className="border-t border-[var(--surface-border)] pt-4">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-fg-strong mb-3">
                        Capabilities
                      </h3>
